...
export const shopMetadata = {
  async myupsertBestSellerProducts(parent, args, ctx: Context, info) {
  ...
  async myupsertNewProducts(parent, args, ctx: Context, info) {
    const shopId = await getShopId(ctx);
  ...
  async myupdateMOTD(parent, args, ctx: Context, info) {
    const shopId = await getShopId(ctx);
