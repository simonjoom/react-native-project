...
const preparedFieldResolvers = addFragmentToFieldResolvers(parse(readFileSync(path.join(__dirname,'../database/datamodel.graphql')).toString()), `{ id }`)
...
const ultimateSchemaString = mergeTypes([
  ...
  ...fileLoader(path.join(__dirname,'./generated'), { extensions: ['.graphql'] }),
