...
class ApolloServer extends apollo_server_core_1.ApolloServerBase {
    ...
    applyMiddleware({ app, path, cors, bodyParserConfig, disableHealthCheck, onHealthCheck, }) {
       ...
       //if (this.uploadsConfig) {
            uploadsMiddleware = fileUploadMiddleware({}, this);
       // }
...
//		app.use(body_parser_1.urlencoded({ extended: true }));
	//	app.use(body_parser_1.text({ type: 'application/graphql' }));
        // app.use("/", body_parser_1.json()); 
...
this.installSubscriptionHandlers(httpServer);
      ...
      const combinedServer = httpServer 
      combinedServer.listen(4000, function(){
