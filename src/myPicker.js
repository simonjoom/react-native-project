...
let {width, height} = {width:200,height:50};
				...
				<View style={styles.pickerWheel} key={index}>
					<Picker
						...
						//console.log(this.pickedValue.splice(index, 1, value))
							...
							// });
							this.props.onValueChange(JSON.parse(JSON.stringify(this.pickedValue)), id);
							this.setState({
	...
	render(){
		...
		let mask = this.props.showMask ? (
			<View style={styles.mask} >
				<Text style={{width: width, height: height}} onPress={this._pickerCancel}></Text>
			</View>
		) : null;
		...
		return (
			<Animated.View style={[styles.picker, {
				...
				<View style={[styles.pickerBox, this.props.style]}>
					<View style={[styles.pickerToolbar, this.props.pickerToolBarStyle, {width: this.props.style.width || width}]}>
						{this.props.onPickerCancel&&<View style={styles.pickerCancelBtn}>
							<Text style={[styles.pickerFinishBtnText, this.props.pickerBtnStyle]} onPress={this._pickerCancel}>{this.props.pickerCancelBtnText}</Text>
						</View>
						}
						<Text style={[styles.pickerTitle, this.props.pickerTitleStyle]} numberOfLines={1}>
							{this.props.pickerTitleText}
						</Text>
						<View style={styles.pickerFinishBtn}>
							<Text style={[styles.pickerFinishBtnText, this.props.pickerBtnStyle]}
								onPress={this._pickerFinish}>{this.props.pickerConfirmBtnText}</Text>
						</View>
					</View>
